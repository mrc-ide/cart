...
par(mfrow = c(1, 3))
terra::plot(data$temporal$`2019`$pop, col = viridis::magma(30), main = "Population")
lines(spatvec_poly, col = "white")
terra::plot(data$temporal$`2019`$pfpr, col = viridis::plasma(30), main = "PfPr", range = c(0, 1))
lines(spatvec_poly, col = "white")
terra::plot(data$vectors$funestus,  main = "Funestus", range = c(0, 1))
lines(spatvec_poly, col = "white")
par(mfrow = c(1, 1))
```

Let's extract temporal data from 2019
```{r, warning=FALSE}
extracted_data <- unpack_cart(sf_poly, data$temporal$`2019`)
```
